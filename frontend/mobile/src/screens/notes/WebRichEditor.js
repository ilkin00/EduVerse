...
<head>
  ...
  <style>
    body { margin: 0; background: #1A1A2E; color: white; }
    #editor-container { height: 100%; background: #1A1A2E; }
    .ql-toolbar { background: #1A1A2E; border-color: #333; }
    .ql-toolbar button { color: white; }
    .ql-toolbar button:hover { color: #6366F1; }
    .ql-container { background: #1A1A2E; color: white; border-color: #333; }
    .ql-editor { color: white; min-height: 300px; }
...
<body>
  ...
  <script>
    ...
    // Matematik sembolleri için özel butonlar
...
const FormulaModal = ({ visible, onClose, onInsert }) => {
  ...
  const templates = [
    ...
    { name: 'Limit', latex: 'lim', preview: 'lim' },
...
export default function WebRichEditor({ route, navigation }) {
        ...
        <>
          ...
          <View style={styles.editorContainer}>
            <WebView
              ...
              onMessage={(event) => setContent(event.nativeEvent.data)}
